...
export const BAD_TV_POST_FX_PIPELINE_KEY = "BadTvPostFxPipeline";

interface BadTvPostFxConfig {
  distortion: number;
  distortion2: number;
  speed: number;
  rollSpeed: number;
...
type BadTvDisplayEffect = Extract<BattleDisplayEffect, "bad-tv">;

const BAD_TV_POST_FX_FRAGMENT_SHADER = `
#define SHADER_NAME PUNGA_BAD_TV_POST_FX
...
void main() {
  ...
  vec2 uv = vec2(fract(p.x + offset), fract(p.y - time * rollSpeed));
  gl_FragColor = texture2D(uMainSampler, uv);
}
`;

const DEFAULT_BAD_TV_CONFIG: BadTvPostFxConfig = {
  distortion: 0,
  distortion2: 0,
  speed: 0,
  rollSpeed: 0,
...
const BAD_TV_CONFIGS: Record<BadTvDisplayEffect, BadTvPostFxConfig> = {
  "bad-tv": {
    distortion: 3,
    distortion2: 5,
    speed: 0.2,
...
export function getBadTvPostFxConfig(effect: BattleDisplayEffect): BadTvPostFxConfig | undefined {
  return effect === "bad-tv" ? BAD_TV_CONFIGS[effect] : undefined;
}
